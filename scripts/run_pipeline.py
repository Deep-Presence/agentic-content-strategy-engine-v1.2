#!/usr/bin/env python3
"""
Run the content-strategy-engine research pipeline: company_research -> style_guide.

Persona research is now a separate pipeline — use scripts/run_audience_persona.py.
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the research pipeline: company -> style_guide.",
    )
    ...
    p.add_argument(
        "--style",
        action="store_true",
        help="Run style_guide stage after company (requires company to complete first)",
    )
    p.add_argument(
        "--auto-approve",
        action="store_true",
...
def main() -> int:
    ...
    if args.style:
        ...
        print("\n" + "=" * 60)
        print("  STAGE 2: Style Guide Research")
        print("=" * 60)
