    ...
    *,
    ...
    for spec in specs_raw:
        ...
        spec_model = ClusterSpecModel(
            ...
            avg_word_count=int((wc_min + wc_max) / 2) if wc_min and wc_max else 0,
            min_similarity_threshold=_safe_float(spec.get("min_similarity_threshold")),
            structural_rates=structural_rates,
            faq_rate=_safe_float(structural_rates.get("faq", 0)),
            table_rate=_safe_float(structural_rates.get("tables", 0)),
            list_rate=_safe_float(structural_rates.get("lists", 0)),
            header_rate=_safe_float(structural_rates.get("headers", 0)),
            stat_rate=_safe_float(structural_rates.get("stats", 0)),
            citation_rate=_safe_float(structural_rates.get("citations", 0)),
        )
