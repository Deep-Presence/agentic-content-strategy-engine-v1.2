"""LangSmith tracing — shared across all pipelines (research, content, etc.).

Unified tracing module providing trace hierarchy, span management, and LLM
generation logging via LangSmith RunTree.
...
def _get_project_name(project_override: Optional[str] = None) -> str:
    """Get the LangSmith project name.

    Args:
        project_override: If provided, use this instead of the default.
    """
    if project_override:
...
def create_pipeline_trace(
    ...
    *,
    ...
    Args:
        ...
        project_name: Override the default LangSmith project name.
...
def create_research_trace(
    ...
    """Create a top-level trace for the research KB pipeline.

    Uses the ``research_kb_project`` setting as the LangSmith project name.

    Args:
        slug: Company (or effective) slug.
        company_name: Human-readable company name.
        metadata: Additional metadata.
    ...
    project = getattr(settings, "research_kb_project", "research-kb")

    try:
        run_tree = RunTree(
            name=f"research-kb-pipeline/{slug}",
            run_type="chain",
