...
interface TaxonomyTreeProps {
  ...
  assignmentsBySubdomain: Map<string, Assignment[]>;
}
...
export function TaxonomyTree({
  ...
  const sortedCategories = useMemo(() => {
    return [...categories].sort((a, b) => {
      const aAvg = getCategoryStats(a, assignmentsBySubdomain).avgScore;
      const bAvg = getCategoryStats(b, assignmentsBySubdomain).avgScore;
      return bAvg - aAvg;
    });
  }, [categories, assignmentsBySubdomain]);

  const sortedChildren = useMemo(() => {
    const map = new Map<string, SubdomainNode[]>();
    for (const cat of sortedCategories) {
      const sorted = [...(cat.children ?? [])].sort(
        (a: SubdomainNode, b: SubdomainNode) => getSortScore(b, sortDimension) - getSortScore(a, sortDimension)
      );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {sortedCategories.map((cat) => {
          ...
          const stats = getCategoryStats(cat, assignmentsBySubdomain);
          ...
          return (
            <div key={cat.id}>
              ...
              {/* Subdomain Nodes */}
              {isOpen &&
                children.map((sub: SubdomainNode) => {
                  const isSelected = sub.id === selectedSubdomainId;
                  const assignments = assignmentsBySubdomain.get(sub.id) ?? [];
